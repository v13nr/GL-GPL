...
$query = "SELECT * FROM ml_user WHERE user = '$user_name' AND status = 1 AND aktif = 1";
...
if (mysql_num_rows($hasil) > 0) {
   ...
   if (md5($passwd) == $data['pass']){
	...
	if (mysql_num_rows($hasil) < 1){
		$browser = Browser::detect();
		$ip = Browser::getRealIpAddr();
		$query = "insert into ml_absen values ('', ".$_SESSION["sess_user_id"].", '".$wkt_disimpan."', '', '".$browser[name]."', '".$browser[version]."', '".$browser[platform]."', '".$ip."')";
		$hasil = mysql_query($query);
	}
	header("location: utama.php");
   }
   else { header("Location: index.php?err=y");  }
}
   else { header("Location: index.php?err=y");  }
