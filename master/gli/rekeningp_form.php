		...
		if($_GET['id']<>""){
			while ($baris=mysql_fetch_array($hasil)) {
				$induk = substr($baris['norek'],0,4);
					$SQLc = "SELECT namarek FROM rek WHERE norek = '$induk' AND status = 1";
					$hasilc = mysql_query($SQLc);
					$barisc = mysql_fetch_array($hasilc);
					$namarekeninginduk = $barisc[0];
				$norek = substr($baris['norek'],-3);
  ...
  <tr>
    ...
    <td><form id="pegForm" method="post" name="pegForm" action="submission_gli.php">
      ...
      <table align="left" class="x1">
        ...
        <tr> 
          <td><span class="style6">Nomor Rekening Pembantu </span></td>
          <td><span class="style6">:</span></td>
          <td><input name="norek" type="text" id="suggest"  maxlength="3" class="required " title="Nomor Rekening Pembantu harus diisi" value="<?=$norek?>" <? if($_GET['id']<>""){?>readonly="true" <? }?> /></td>
        </tr>
        <tr>
          <td><span class="style6">Nama Rekening </span></td>
          <td><span class="style6">:</span></td>
