...
$SQL = "select * from dbfj where sub = '".$_GET['divisi']."' and nobukti = '".$_GET['nobukti']."'";
$hasil = mysql_query($SQL);
$baris = mysql_fetch_array($hasil);
	if($baris["jenis"]=="Kredit"){
		$norek = $baris["kk"];
			$SQLc = "SELECT namarek FROM dbfm WHERE norek = '".$baris["kk"]."'";
	...
	if($baris["jenis"]=="Debet"){
		$norek = $baris["kd"];
			$SQLc = "SELECT namarek FROM dbfm WHERE norek = '".$baris["kd"]."'";
			$hasilc = mysql_query($SQLc);
			$barisc = mysql_fetch_array($hasilc);
		$namarek = $barisc[0];
		$uraian = $baris["ket"];
...
$pdf->cell(130,6,'SLIP TRANSAKSI ', 1, 0, 'C');
$pdf->setFont('Arial','',10);
$pdf->cell(30,6,'TANGGAL', 1, 0, 'C');
$pdf->setY(20);	
$pdf->cell(30,6,$_GET['divisi'], 1, 0, 'C');
$pdf->cell(30,6,$norek, 1, 0, 'L');
$pdf->cell(100,6,$namarek, 1, 0, 'L');
$pdf->cell(30,6,$_GET['tanggal'], 1, 0, 'C');
$pdf->setY(26);	
$pdf->cell(30,6,'', 1, 0, 'C');
$pdf->cell(30,6,'Uraian', 1, 0, 'L');
$pdf->cell(100,6,$uraian, 1, 0, 'L');
$pdf->setFont('Arial','B',10);
$pdf->cell(30,6,'No. '.$_GET['divisi'].'/'.nobukti($_GET['nobukti']), 1, 0, 'C');
$pdf->setY(35);	
...
while($baris = mysql_fetch_array($hasil)){
	...
	if($baris["jenis"]=="Kredit"){
		$pdf->cell(20,6,$baris["kd"], 1, 0, 'C');
			...
			$hasilc = mysql_query($SQLc);
			$barisc = mysql_fetch_array($hasilc);
		$pdf->cell(40,6,$barisc[0], 1, 0, 'L');
		$pdf->cell(75,6,$baris["ket"], 1, 0, 'L');			
	}
	if($baris["jenis"]=="Debet"){
		$pdf->cell(20,6,$baris["kk"], 1, 0, 'C');
			...
			$barisc = mysql_fetch_array($hasilc);
		$pdf->cell(40,6,$barisc[0], 1, 0, 'L');
		$pdf->cell(75,6,$baris["ket2"], 1, 0, 'L');			
	}	
