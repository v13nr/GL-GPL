...
<input type="text" name="tgl_akhir" id="tgl_akhir"  value="<?=$_GET['tgl_akhir']?>" size="10" class="required" title="Harap Mengisi Tanggal Akhir Dahulu" />
    ...
    <tr <?	 if (($nRecord % 2)==0) {?>bgcolor="#e4e4e4"<? }  else {?>bgcolor="#FFFFCC"<? } ?>>
      ...
      <td class="style3" align="center"><a href="cetak_pdf.php?divisi=<?=$row['sub']?>&nobukti=<?=$row['nobukti']?>&tanggal=<?=baliktglindo($row['tanggal'])?>"><?=$row['sub']?>/<?=nobukti($row['nobukti'])?>/<?=substr($row['bulan'],0,2)?></a></td>
	  ...
	  <?
	  	if($row['jenis']=="Debet"){
			$norek=$row['kd'];
				$SQLc = "SELECT namarek FROM dbfm WHERE norek = '".$row['kd']."'";
			...
			$norek=$row['kk'];
				$SQLc = "SELECT namarek FROM dbfm WHERE norek = '".$row['kk']."'";
				$hasilc = mysql_query($SQLc);
				$barisc = mysql_fetch_array($hasilc);
			$namarek=$barisc[0];
			$keteranganheader= $row['ket2'];
		}
		$strurl = "index.php?mn=trans_jurnal&nobukti=".$row['nobukti']."&tgl_transaksi=".baliktglindo($row['tanggal'])."&dk=".$row['jenis']."&norek=".$norek."&namarek=".$namarek."&keteranganheader=".$keteranganheader."&bulan=".$row['bulan'];
	  ?>
	  <td align="center">
	  	<? if($_SESSION["sess_user_id"]==$row['user_id'] || $_SESSION["sess_kelasuser"] == "Super Admin" || $_SESSION["sess_kelasuser"] == "Admin") { ?>
	  <a href="<?=$strurl?>"><img src="../draft/images/user_go.png" border="0" width="16" height="16" /></a>
	  <? } ?>
	  </td>
    </tr>
